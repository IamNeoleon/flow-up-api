...
export class ColumnService {
    ...
    async getAll(boardId: string) {
        const existBoard = await this.prismaService.board.findUnique({
            where: { id: boardId },
        });
        if (!existBoard)
            throw new NotFoundException('Доска с таким id не найдена');
    ...
    async create(
        ...
        if (!canCreateColumn({ workspaceRole })) {
    ...
    async update(
        ...
        if (!canUpdateColumn({ workspaceRole })) {
            throw new ForbiddenException();
        }
    ...
    async delete(id: string, boardId: string, workspaceRole: TWorkspaceRole) {
        if (!canDeleteColumn({ workspaceRole })) {
            throw new ForbiddenException();
        }
    ...
    async changeOrder(
        ...
        if (!canMoveColumn({ workspaceRole })) {
            throw new ForbiddenException();
        }
        ...
        const column = await this.prismaService.$transaction(async (tx) => {
            ...
            if (!column)
                throw new NotFoundException('Не найдена заданная колонка');
