...
export class TaskService {
    ...
    async create(args: TaskCreateArgs) {
        const { boardId, colId, userId, workspaceRole, workspaceId, dto } =
            args;

        if (!canCreateTask({ workspaceRole })) {
    ...
    async update(args: TaskUpdateArgs) {
        ...
        if (
            existTask.assigneeId === updated.assigneeId &&
            existTask.priorityId !== updated.priorityId &&
            updated.priority &&
            updated.assigneeId
        ) {
            this.notificationService.createNotification({
        ...
        this.eventEmitter.emit('TASK_UPDATED', {
            colId,
            boardId,
            actorId: userId,
            taskId,
    ...
    async delete(
        ...
        if (!canDeleteTask({ workspaceRole })) {
            throw new ForbiddenException();
        }

        const task = await this.getTaskOrFail(taskId);

        const deletedTask = await this.prismaService.$transaction(
            async (tx) => {
    ...
    async moveTask(args: {
        ...
        const movedTask = await this.prismaService.$transaction(async (tx) => {
            ...
            const requestedOrder =
                newOrder ??
                (isSameColumn
                    ? targetColumnTasksCount
                    : targetColumnTasksCount + 1);

            const safeOrder =
                targetColumnTasksCount === 0
                    ? 1
                    : isSameColumn
                      ? Math.max(
                            1,
                            Math.min(requestedOrder, targetColumnTasksCount),
                        )
                      : Math.max(
                            1,
                            Math.min(
